...
export class OnyxChatWidget extends LitElement {
  ...
  @state() private starterMessages: StarterMessage[] = [];
  ...
  connectedCallback() {
    ...
    void this.loadStarterMessages();
  }

  private async loadStarterMessages() {
    if (this.config.agentId === undefined) {
      return;
    }

    try {
      const persona = await this.apiService.getPersona(this.config.agentId);
      this.starterMessages = persona.starter_messages ?? [];
    } catch (err) {
      console.error("Failed to load starter messages:", err);
    }
  }
  ...
  private renderCitationBadge(
    ...
    const title = c.semantic_identifier || "Quelle";
    const safeHref =
  ...
  private async sendMessage() {
    ...
    try {
      ...
      if (err.name !== "AbortError") {
        this.error = err.message || "Nachricht konnte nicht gesendet werden";
      }
    } finally {
      this.isStreaming = false;
      this.isLoading = false;
      ...
      <div class="header">
        <div class="header-left">
          ...
          <div class="header-title">
            ${this.config.agentName || "Assistent"}
          </div>
        </div>
        <div class="header-right">
          ...
          ${this.config.mode === "launcher"
            ? html`
                <button class="icon-button" @click=${this.close} title="Schließen">
                  <svg
                    width="16"
                    height="16"
                    viewBox="0 0 28 28"
  ...
  private renderStarterMessages() {
    if (this.starterMessages.length === 0) {
      return "";
    }

    return html`
      <div class="starter-messages">
        ${this.starterMessages.map(
          (starter) => html`
            <button
              class="starter-message"
              @click=${() => this.selectStarterMessage(starter.message)}
    ...
    return html`
      <div class="compact-input-container">
        ...
        <input
          ...
          @input=${this.handleInput}
          @keydown=${this.handleKeyDown}
          placeholder="Stelle ${this.config.agentName || "Assistent"} eine Frage..."
          ?disabled=${this.isLoading || this.isStreaming}
        />
        <button
          class="send-button"
